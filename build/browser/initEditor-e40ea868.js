...
const colorRegex = new RegExp(/(bg|text|border|ring)-(red|yellow|green|blue|indigo|purple|green|white)-(\d\d\d)/, "g");
...
const loadTemplate = async (editor, standaloneServer) => {
  const baseUrl = standaloneServer ? `http://localhost:${index.standaloneServerPort}` : "";
  const data = await index.fetchJSON({ method: "get", url: `${baseUrl}/api/builder/handle` });
  ...
  const component = Object.keys(data).find((c) => [pathNameWindows, pathNameUnix].includes(data[c].filename));
  if (!component)
    return;
